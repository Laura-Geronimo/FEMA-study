##Pulled Data for COUNTIES with Tidyverse

#redid 07/11/2024

##Prior NOTES:
  #Tidyverse does not seem to like it when I pull data just for a vector of countys in my dataset
  #Workaround: pull data for specific variables by specific year for all countys, then join to dataset
  #For variable names, using: https://api.census.gov/data/2019/acs/acs5/profile/variables.html
  #Other links Will mentioned:
    #https://www.census.gov/programs-surveys/geography/guidance/geo-areas/countys.html
...
y2000dec_sf3 <- get_decennial(geography = "county", 
                      variables = c(
...
#2010 Decennial ####
y2010dec <- get_decennial(geography = "county", 
                          variables = c(
...
#2011 ACS####
y2011acs <-get_acs(geography= "county",
                variables = c(
...
#2012 ACS ####
y2012acs <-get_acs(geography= "county",
                   variables = c(
...
#2013 ACS ####
y2013acs <-get_acs(geography= "county",
                   variables = c(
...
#2014 ACS ####
y2014acs <-get_acs(geography= "county",
                   variables = c(
...
#2015 ACS ####
y2015acs <-get_acs(geography= "county",
                   variables = c(
...
#2016 ACS ####
y2016acs <-get_acs(geography= "county",
                   variables = c(
...
#2017 ACS ####
y2017acs <-get_acs(geography= "county",
                   variables = c(
...
#2018 ACS ####
y2018acs <-get_acs(geography= "county",
                   variables = c(
...
#2019 ACS ####
y2019acs <-get_acs(geography= "county",
                   variables = c(
...
#2020 ACS ####
y2020acs <-get_acs(geography= "county",
                   variables = c(
...
#2021 ACS ####
y2021acs <-get_acs(geography= "county",
                   variables = c(
...
TS_county_0 <- left_join(y2000dec_2, y2010dec_2, by="CntyID", copy=F)
TS_county_1 <- left_join(TS_county_0, y2011acs_2, by="CntyID", copy=F)
TS_county_2 <- left_join(TS_county_1, y2012acs_2, by="CntyID", copy=F)
TS_county_3 <- left_join(TS_county_2, y2013acs_2, by="CntyID", copy=F)
TS_county_4 <- left_join(TS_county_3, y2014acs_2, by="CntyID", copy=F)
TS_county_5 <- left_join(TS_county_4, y2015acs_2, by="CntyID", copy=F)
TS_county_6 <- left_join(TS_county_5, y2016acs_2, by="CntyID", copy=F)
TS_county_7 <- left_join(TS_county_6, y2017acs_2, by="CntyID", copy=F)
TS_county_8 <- left_join(TS_county_7, y2018acs_2, by="CntyID", copy=F)
TS_county_9 <- left_join(TS_county_8, y2019acs_2, by="CntyID", copy=F)
TS_county_10 <- left_join(TS_county_9, y2020acs_2, by="CntyID", copy=F)
