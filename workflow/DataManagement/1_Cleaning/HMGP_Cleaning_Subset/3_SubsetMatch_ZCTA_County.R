...
table(is.na(myHMA7$myZflag)) #1835
myHMA7$myZflag[is.na(myHMA7$myZflag)]<-0
table(myHMA7$myZflag)

seeMissingZips <- subset(myHMA7, myZflag==0)
table(seeMissingZips$propertyAction3) #loosing about 1650 Acquisitions, and 185 elevations
...
table(is.na(SeeCnty$p2countyCode)) #419 missing
head(SeeCnty)
SeeNAs <- SeeCnty[rowSums(is.na(SeeCnty))>0,] ##436 missing
table(SeeNAs$p2state) 
uniqueCO <- distinct(SeeNAs) #25 distinct missing. 


myHMA8$CntyID <- myHMA8$p2countyCode
table(is.na(myHMA8$CntyID)) #420
myHMA8$CntyID <- str_pad(myHMA8$CntyID , 3, pad="0")
head(myHMA8$CntyID)


head(myHMA8$stateNumberCode)
table(is.na(myHMA8$stateNumberCode)) #28
...
table(is.na(myHMA8$CntyID)) #436 missing
...
table(seeMissingCnty$propertyAction3) #loosing 132 elevations, 916 acquisitions
names(seeMissingCnty)
seeMissingCnty2 <- seeMissingCnty[,c(6,7,8,9)]
seeMissingCnty2 <- distinct(seeMissingCnty2) #35
...
MissingElev <- aggregate(interp3$Elev~interp3$cntySt, FUN=sum) #3 over 10 
MissingAcqui <- aggregate(interp3$Acqui~interp3$cntySt, FUN=sum) #20 over 10
...
table(is.na(myHMA8b$CntyID)) #436 missing
...
#from prior analysis
#myHMA8b$CntyID2 <- ifelse(myHMA8b$p2state=="Puerto Rico" & myHMA8b$county=="Patillas", "72109", myHMA8b$CntyID2)
#myHMA8b$CntyID2 <- ifelse(myHMA8b$p2state=="Illinois" & myHMA8b$county=="De Witt", "17039", myHMA8b$CntyID2)
...
table(myHMA8b$CntyID3) #Now 133 0s --> converting back to NAs
myHMA8b$CntyID3[myHMA8b$CntyID3==0]<-NA
table(is.na(myHMA8b$CntyID3)) #140
...
table(seeNAs$propertyAction3) #130 buyouts, 10 elevations
...
table(seeMissingCnty$propertyAction3) #now only missing 178 buyouts, 42 elevs
...
names(myHMA9c)
table(myHMA9c$propertyAction3) #now have 10972 elevations, 49821 buyouts
